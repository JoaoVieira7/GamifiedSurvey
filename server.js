...
app.post("/submit", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Dados submetidos com sucesso", redirectUrl: "/final" });
  } catch (err) {
    console.error("Erro ao inserir dados:", err);
    res
      .status(500)
      .json({ message: "Erro ao inserir dados no banco de dados", error: err });
  }
});
...
app.post("/leave", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Erro ao registrar dados no banco de dados",
      error: err,
    });
  }
});
...
app.get("/retention-data", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM retention_data_gamified ORDER BY id ASC "
    );
    res.json(result.rows);
  } catch (err) {
    console.error("Erro ao buscar dados:", err);
    res
      .status(500)
      .json({ message: "Erro ao buscar dados do banco de dados", error: err });
  }
});
...
app.get("/post", (req, res) => {
  res
    .status(404)
    .send(
      "Endpoint POST não existe. Use o método POST para enviar dados para /submit."
    );
});
